...
public class UserCenterController {
	...
	@GetMapping("/findAddr/{enc}")
	public Result findAddressByEnc(@PathVariable String enc) {
		List<Address> list = userCenterService.findAddressByEnc(enc);
		return new Result(true,StatusCode.OK,"查询成功",list);
	}
	
	@PostMapping("/update/{enc}")
	public Result updateInfo(@PathVariable String enc,@RequestBody User user) {
		userCenterService.updateUserInfo(enc, user);
