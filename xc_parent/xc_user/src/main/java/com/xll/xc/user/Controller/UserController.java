...
/**
 * ClassName : UserController Description : 用户控制层 CreateDate : 2019年5月28日
 * 下午9:59:02 Author : Yudachi
 * 
 * @Version : V1.0.0
 */
@RestController
...
public class UserController {
	...
	public Result sellerLogin(@RequestBody Seller seller) {
		...
		// 生成JWT令牌
		String token = jwtUtil.createJWT(loginSeller.getSeller_id(), loginSeller.getName(), "seller");
		Map<String, Object> map = new HashMap<>();
		map.put("token", token);
		map.put("id", loginSeller.getSeller_id());
		map.put("username", loginSeller.getName());
		map.put("role", "seller");
