...
$(document)
		.ready(
				function() {
					...
					$('li.hd_menu_tit')
							.mousemove(
									function() {
										$(this)
												.find(
														'div.hd_menu_list,div.hd_Shopping_list')
												.show();// you can give it a
														// speed
									});
					$('li.hd_menu_tit')
							.mouseleave(
					...
					// 文本框登录注册事件
					$('#user_text, #tbPassword, #confirm_pwd_text, #Codes_text')
							.blur(
									function() {
										var $el = $(this);
					...
					$('.check_agreement')
							.toggle(
									function() {
										...
										$('.clearfix').find('.auto_tips')
												.removeClass('on-focus').attr(
														'class', 'auto_tips')
												.addClass(' on-focus');
...
$(window).scroll(function() {
	/*
	 * var flag=1; $('#rightArrow').click(function(){ if(flag==1){
	 * $("#floatDivBoxs").animate({left: '-175px'},300); $(this).animate({left:
	 * '-5px'},300); $(this).css('background-position','-50px 0'); flag=0;
	 * }else{ $("#floatDivBoxs").animate({left: '0'},300);
				...
				function() {
					(function($) {
							...
							this
									.each(function() {
										var $el = $(this), sMethod = ($el
												.is(":input") ? (defaults.useFieldPlugin ? "getValue"
												: "val")
												: "text"), v = $.trim(
												$el[sMethod]()).match(
												defaults.reNumbers, "");
										if (v == null) {
											v = 0;
											if (jQuery
													.isFunction(options.onParseError))
												options.onParseError.apply($el,
														[ sMethod ]);
											$.data($el[0], "calcParseError",
													true);
										} else {
											v = options.cleanseNumber.apply(
													this, [ v[0] ]);
											if ($
													.data($el[0],
															"calcParseError")
													&& jQuery
															.isFunction(options.onParseClear)) {
												options.onParseClear.apply($el,
														[ sMethod ]);
												$
														.data(
																$el[0],
						...
						$.fn.calc = function(expr, vars, cbFormat, cbDone) {
							...
							for ( var k in vars) {
								expr = expr.replace((new RegExp("(" + k + ")",
										"g")), "_.$1");
							...
							this
									.each(function(i, el) {
										...
										try {
											exprValue = eval(expr);
											if (precision)
												exprValue = Number(exprValue
														.toFixed(Math.max(
																precision, 4)));
											if (jQuery.isFunction(cbFormat)) {
												var tmp = cbFormat.apply(this,
														[ exprValue ]);
												if (!!tmp)
													exprValue = tmp;
											}
								...
								.each(
										...
										function(i, method) {
											$.fn[method] = function(bind,
												...
												var opt = bind
														&& bind.constructor == Object ? bind
														: {
															bind : bind
																	|| "keyup",
															selector : (!bSelOpt) ? selector
																	: null,
															oncalc : null
														};
												if (bSelOpt)
												...
												var self = this, sMethod, doCalc = function() {
													var value = math[method]
															(self
																	.parseNumber(opt));
													if (!!opt.selector) {
														sMethod = (opt.selector
																.is(":input") ? (defaults.useFieldPlugin ? "setValue"
																: "val")
																: "text");
														opt.selector[sMethod]
																(value
																		.toString());
													}
													if (jQuery
															.isFunction(opt.oncalc))
														opt.oncalc.apply(self,
																[ value, opt ]);
												};
												doCalc();
												return self.bind(opt.bind,
														doCalc);
											}
										});
						var math = {
							sum : function(a) {
								...
								$
										.each(
												a,
												function(i, v) {
													var p = v.toString().match(
															/\.\d+$/gi), len = (p) ? p[0].length - 1
															: 0;
													if (len > precision)
														precision = len;
													total += v;
								...
								.calc(
										...
										function($this) {
											...
											$("#total_price")
													.val(
															$(
																	"[id^=total_item_1],[id^=total_item_2]")
																	.text());
										});
